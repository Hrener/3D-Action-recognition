...
class ConVNet(nn.Module):
    def __init__(self, in_dim=25, out_dim=30):
        ...
        self.ConVNet_up = nn.Sequential(  # 3*30*30
            nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3),  # 32*28*28
            nn.MaxPool2d(stride=2, kernel_size=2),  # 32*14*14
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3),  # 64*12*12
            nn.ReLU(),
            nn.MaxPool2d(stride=2, kernel_size=2),  # 128*5*5
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3),  # 128*10*10
            nn.ReLU(),
        )
